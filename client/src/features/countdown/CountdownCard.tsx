...
export function CountdownCard({ item, onDelete }: Props) {
  ...
  return (
    <div className="bg-white rounded-card shadow p-6 text-center relative group hover:shadow-lg hover:-translate-y-1 transition-all">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-primary to-primary-light rounded-t-card" />
      <button onClick={onDelete} className="absolute top-3 right-3 text-gray-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">&times;</button>
      <span className="text-4xl">{item.icon}</span>
      <h3 className="text-lg font-semibold mt-2 mb-3">{item.title}</h3>
      {time.expired ? <p className="text-gray-400">已过期</p> : (
        <div className="flex justify-center gap-3 flex-wrap">
          {[{ v: time.days, l: '天' }, { v: time.hours, l: '时' }, { v: time.minutes, l: '分' }, { v: time.seconds, l: '秒' }].map((u) => (
            <div key={u.l} className="text-center">
              ...
              <span className="text-xs text-gray-400">{u.l}</span>
            </div>
          ))}
        </div>
      )}
