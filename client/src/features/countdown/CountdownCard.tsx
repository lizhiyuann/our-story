...
export function CountdownCard({ item, onDelete }: Props) {
  ...
  useEffect(() => {
    const timer = setInterval(() => setTime(calcCountdown(item.targetDate)), 1000);
  ...
  return (
    <div className="bg-white rounded-card shadow p-6 text-center relative group hover:shadow-lg hover:-translate-y-1 transition-all">
      ...
      {time.expired ? (
        <p className="text-gray-400">已过期</p>
      ) : (
        <div className="flex justify-center gap-3 flex-wrap">
          {[
            ...
            <div key={u.l} className="text-center">
              <span className="block text-2xl font-bold text-primary bg-love-bg px-3 py-1 rounded-card min-w-[50px]">
                {u.l === '天' ? u.v : pad(u.v)}
              </span>
