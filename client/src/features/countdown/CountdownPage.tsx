...
const ICONS = [
  { value: '🎂', label: '生日' }, { value: '💕', label: '纪念日' },
  { value: '🎄', label: '节日' }, { value: '✈️', label: '旅行' },
  { value: '🎓', label: '毕业' }, { value: '💍', label: '求婚' },
  { value: '❤️', label: '其他' },
];
...
export function CountdownPage() {
  ...
  const handleSubmit = () => {
    ...
    createCountdown.mutate({ title: title.trim(), targetDate: date, icon }, {
      onSuccess: () => { setTitle(''); setDate(''); showToast('倒计时已添加 ⏰'); },
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2">
          {isLoading ? <LoadingState /> : isError ? <EmptyState icon="⚠️" message="加载失败，请刷新重试" /> : items.length === 0 ? <EmptyState icon="⏰" message="还没有倒计时哦~" /> : (
            <div className="grid sm:grid-cols-2 gap-4">
              {items.map((item) => <CountdownCard key={item.id} item={item} onDelete={() => { if (window.confirm('确定删除这个倒计时吗？')) deleteCountdown.mutate(item.id); }} />)}
            </div>
          )}
        </div>
      </div>
